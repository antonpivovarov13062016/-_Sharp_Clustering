...
namespace WFASeasonalCoefficients
{
    partial class AboutBox1 : Form
    {
        ...
        private void buttonDescription_Click(object sender, EventArgs e)
        {
            ...
            Process.Start("Description.doc");
            var runProc = from proc in Process.GetProcesses(".") orderby proc.Id select proc;
            int rPCount = runProc.Count(p => p.ProcessName.Contains("WORD") || p.ProcessName.Contains("word"));
            if (rPCount <= 0)
            {
        ...
        {
            Process.Start("Instruction.doc");
            var runProc = from proc in Process.GetProcesses(".") orderby proc.Id select proc;
            int rPCount = runProc.Count(p => p.ProcessName.Contains("WORD") || p.ProcessName.Contains("word"));
            if (rPCount <= 0)
            {
                Process.Start("Instruction.txt");
            }
        }
    }
}
